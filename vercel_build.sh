...
set +e
flutter build web --release --base-href / --no-tree-shake-icons --no-wasm-dry-run "${DART_DEFINES[@]}"
BUILD_STATUS=$?
set -e
...
if [ "$BUILD_STATUS" -ne 0 ]; then
  ...
  rm -rf build
  flutter build web --release --base-href / --no-tree-shake-icons "${DART_DEFINES[@]}"
fi
