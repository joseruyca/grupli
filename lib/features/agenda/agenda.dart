...
class _EventsTabState extends State<EventsTab> {
      ...
      child: Stack(children: [
        ListView(
          ...
          children: [
            PageHeader(
                title: 'Eventos',
                subtitle: AppData.text(group['name']),
                leading: true),
            ...
            FutureBuilder<List<Map<String, dynamic>>>(
              ...
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting)
                  return const CenterLoader(label: 'Cargando eventos...');
                if (snapshot.hasError)
                  return ErrorBlock(message: snapshot.error, onRetry: reload);
                final events = snapshot.data ?? [];
                final upcoming = events
                    .where((e) =>
                        DateTime.tryParse(e['starts_at']?.toString() ?? '')
                ...
                return Column(
                    ...
                    children: [
                      Row(children: [
                        ...
                        Expanded(
                            child: StatCard(
                                ...
                                value:
                                    _attendanceTotal(events, 'yes').toString(),
                                label: 'Confirmados',
                      ...
                      Row(children: [
                        ...
                        const Spacer(),
                        TextButton(
                            onPressed: reload, child: const Text('Actualizar'))
                      ]),
                      ...
                      if (upcoming.isEmpty)
                        EmptyBlock(
                            ...
                            body:
                                'Crea una quedada para que los miembros confirmen asistencia.')
                      else
  ...
  int _attendanceTotal(List<Map<String, dynamic>> events, String status) {
    ...
    for (final e in events) {
      ...
      if (att is List)
        total += att.where((x) => (x as Map)['status'] == status).length;
    }
    return total;
  }
}
    ...
    _debounce = Timer(const Duration(milliseconds: 420), () async {
      try {
        ...
        setState(() {
          ...
          loading = false;
          error =
              'No se pudo buscar. Puedes escribir la dirección manualmente.';
        });
      }
    });
  }
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      TextField(
        ...
        decoration: InputDecoration(
          ...
          helperText:
              'Autocompleta con OpenStreetMap. También puedes escribirlo a mano o pegar un enlace de Maps.',
          helperMaxLines: 2,
        ),
      ),
                  ...
                  child: Row(children: [
                    ...
                    Expanded(
                        child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                          Text(suggestions[i].description,
                              maxLines: 2,
    ...
    return AppCard(
      ...
      child: Row(children: [
        Container(
            ...
            child: const Icon(Icons.map_rounded, color: AppColors.teal)),
        ...
        TextButton.icon(
          onPressed: () {
            openAddressInGoogleMaps(context, clean);
          },
          icon: const Icon(Icons.navigation_rounded, size: 17),
          label:
              const Text('Maps', style: TextStyle(fontWeight: FontWeight.w900)),
...
class CreateEventScreen extends StatefulWidget {
  ...
  const CreateEventScreen(
      {super.key, required this.group, this.initialDate, this.event});
...
class _CreateEventScreenState extends State<CreateEventScreen> {
  ...
  void initState() {
    ...
    if (title.text.toLowerCase().contains('entrenamiento'))
      template = 'Entrenamiento';
    if (title.text.toLowerCase().contains('cena')) template = 'Cena';
    if (title.text.toLowerCase().contains('reunión') ||
        title.text.toLowerCase().contains('reunion')) template = 'Reunión';
  }
  ...
  void applyTemplate(String value) {
    setState(() {
      ...
      if (title.text.trim().isEmpty ||
          ['Quedada', 'Partido', 'Entrenamiento', 'Cena del grupo', 'Reunión']
  ...
  Future<void> save() async {
    ...
    if (cleanTitle.length < 2) {
      await showToast(context, 'Pon un título claro para el evento.',
          danger: true);
      return;
    }
    if (!editing && repeatEnabled && repeatOccurrences < 2) {
      await showToast(context, 'Una rutina necesita al menos 2 eventos.',
          danger: true);
      return;
    }
    ...
    try {
      if (editing) {
        await AppData.updateEventWithScope(widget.event!['id'].toString(),
            editScope, cleanTitle, start, location.text, notes.text, minPeople);
        ...
        if (mounted)
          await showToast(
              context, 'Rutina creada: $created eventos generados.');
      } else {
        await AppData.createEvent(widget.group['id'].toString(), cleanTitle,
            start, location.text, notes.text, minPeople);
      }
    ...
    return DirectPage(
      ...
      EventFormPreviewCard(
        title: title.text.trim().isEmpty
            ? (editing ? 'Evento del grupo' : 'Nueva quedada')
            : title.text.trim(),
        date: previewDate,
        location: location.text.trim(),
        minPeople: minPeople,
        template: template,
        repeatLabel: repeatEnabled
            ? routinePreview
            : (editingRoutine ? eventRoutineBadge(widget.event!) : null),
      ),
      if (editingRoutine) ...[
        const SizedBox(height: 14),
        EventScopeCard(
      ...
      Wrap(spacing: 8, runSpacing: 8, children: [
        EventTemplateChoice(
            label: 'Quedada',
            icon: Icons.groups_rounded,
            selected: template == 'Quedada',
            onTap: () => applyTemplate('Quedada')),
        EventTemplateChoice(
            label: 'Partido',
            icon: Icons.sports_soccer_rounded,
            selected: template == 'Partido',
            onTap: () => applyTemplate('Partido')),
        EventTemplateChoice(
            label: 'Entrenamiento',
            icon: Icons.fitness_center_rounded,
            selected: template == 'Entrenamiento',
            onTap: () => applyTemplate('Entrenamiento')),
        EventTemplateChoice(
            label: 'Cena',
            icon: Icons.restaurant_rounded,
            selected: template == 'Cena',
            onTap: () => applyTemplate('Cena')),
        EventTemplateChoice(
            label: 'Reunión',
            icon: Icons.forum_rounded,
            selected: template == 'Reunión',
            onTap: () => applyTemplate('Reunión')),
      ]),
      const SizedBox(height: 16),
      AppCard(
          child:
        ...
        TextField(
            ...
            decoration: const InputDecoration(hintText: 'Ej. Partido semanal')),
        const SizedBox(height: 14),
        Row(children: [
          Expanded(
              child: SmallPick(
                  label: 'Fecha',
                  value: DateFormat('dd/MM/yyyy', appDateLocale).format(date),
                  icon: Icons.calendar_month_rounded,
                  onTap: () async {
          ...
          child:
            ...
            Row(children: [
              Container(
                ...
                height: 42,
                decoration: BoxDecoration(
                    color: repeatEnabled ? AppColors.teal : AppColors.surface,
                    borderRadius: BorderRadius.circular(15),
                    border: Border.all(color: AppColors.line)),
                child: Icon(Icons.repeat_rounded,
                    color: repeatEnabled ? Colors.white : AppColors.teal,
                    size: 22),
              ),
              const SizedBox(width: 12),
              Expanded(
                  child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                    Text('Convertir en rutina',
                        style: Theme.of(context).textTheme.titleMedium),
                    const SizedBox(height: 3),
                    Text(
                        'Ideal para partido todos los jueves, entreno semanal o quedadas fijas.',
            ...
            if (repeatEnabled) ...[
              ...
              Wrap(spacing: 8, runSpacing: 8, children: [
                RoutineChoice(
                    label: 'Cada semana',
                    selected: repeatFrequency == 'weekly',
                    onTap: () => setState(() => repeatFrequency = 'weekly')),
                RoutineChoice(
                    label: 'Cada 2 semanas',
                    selected: repeatFrequency == 'biweekly',
                    onTap: () => setState(() => repeatFrequency = 'biweekly')),
                RoutineChoice(
                    label: 'Cada mes',
                    selected: repeatFrequency == 'monthly',
                    onTap: () => setState(() => repeatFrequency = 'monthly')),
              ]),
              const SizedBox(height: 14),
              Row(children: [
                Expanded(
                ...
                Container(
                  ...
                  child: Row(mainAxisSize: MainAxisSize.min, children: [
                    IconButton(
                        onPressed: () => setState(() =>
                            repeatOccurrences = max(2, repeatOccurrences - 1)),
                        icon: const Icon(Icons.remove_rounded)),
                    Text(repeatOccurrences.toString(),
                        style: const TextStyle(
                            fontSize: 18,
                    ...
                    IconButton(
                        onPressed: () => setState(() =>
                            repeatOccurrences = min(52, repeatOccurrences + 1)),
              ...
              RoutineInfoBox(
                  text:
                      'Se crearán $repeatOccurrences fechas conectadas en una misma rutina. Después podrás editar solo una fecha, esta y futuras, o toda la rutina.'),
            ],
          ]),
        ),
      ],
      ...
      AppCard(
        ...
        Row(children: [
          ...
          Expanded(
              child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                Text('Mínimo para que el plan salga adelante',
                    style: Theme.of(context).textTheme.titleMedium),
                const SizedBox(height: 3),
                Text(
                    'Grupli avisará cuando haya suficientes personas confirmadas.',
                    style: Theme.of(context).textTheme.bodyMedium),
              ])),
        ]),
        const SizedBox(height: 12),
        StepperRow(
            value: minPeople,
            onMinus: () => setState(() => minPeople = max(1, minPeople - 1)),
            onPlus: () => setState(() => minPeople++)),
      ])),
      const SizedBox(height: 14),
      ...
      TextField(
          ...
          decoration: const InputDecoration(
              hintText:
                  'Material, normas, instrucciones, coste aproximado...')),
      const SizedBox(height: 22),
      PrimaryButton(
        label: editing
            ? 'Guardar cambios'
            : (repeatEnabled ? 'Crear rutina' : 'Crear evento'),
        icon: editing
            ? Icons.save_rounded
            : (repeatEnabled ? Icons.repeat_rounded : Icons.add_rounded),
        loading: loading,
        onTap: save,
      ),
    ]));
...
class EventDetailScreen extends StatefulWidget {
  ...
  const EventDetailScreen(
      {super.key, required this.event, required this.group});
...
class _EventDetailScreenState extends State<EventDetailScreen> {
  ...
  void load() {
    future = _EventDetailData.load(
        widget.event['id'].toString(), widget.group['id'].toString());
  }
  ...
  Future<void> saveContribution(Map<String, dynamic> event,
      [Map<String, dynamic>? current]) async {
    final initial = AppData.text(current?['items_text']);
    final value = await showEventContributionDialog(context,
    ...
    try {
      ...
      reload();
      if (mounted)
        await showToast(context, 'Guardado. El grupo ya puede ver qué llevas.');
    } catch (e) {
      await showToast(context, humanError(e), danger: true);
    } finally {
      if (mounted) setState(() => saving = false);
  ...
  Future<void> editEvent(Map<String, dynamic> event) async {
    final ok = await Navigator.of(context).push<bool>(MaterialPageRoute(
        builder: (_) => CreateEventScreen(group: widget.group, event: event)));
    if (ok == true) reload();
  }
  ...
  Future<void> cancelEvent(Map<String, dynamic> event) async {
    ...
    if (isRoutine) {
      final selectedScope = await showRoutineScopeDialog(context,
          title: 'Cancelar rutina', actionLabel: 'Cancelar');
      ...
      final yes = await showDialog<bool>(
        ...
        builder: (_) => AlertDialog(
          ...
          content: const Text(
              'El evento dejará de aparecer en el calendario del grupo. Esta acción no borra el grupo.'),
          actions: [
            TextButton(
                onPressed: () => Navigator.pop(context, false),
                child: const Text('No')),
  ...
  Future<void> openLinkedTournament(Map<String, dynamic> event) async {
    ...
    try {
      final match =
          await AppData.tournamentMatchByEventId(event['id'].toString());
      if (!mounted) return;
      final tournamentId = AppData.text(match?['tournament_id']);
      if (tournamentId.isEmpty) {
        await showToast(
            context, 'Este evento de Agenda no encuentra su torneo vinculado.',
            danger: true);
        return;
      }
      await Navigator.of(context).push(MaterialPageRoute(
        builder: (_) => TournamentDetailSimpleScreen(
            tournamentId: tournamentId, group: widget.group),
      ));
      reload();
    } catch (e) {
      ...
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          return const DirectPage(
              child: CenterLoader(label: 'Cargando evento...'));
        }
        if (snapshot.hasError) {
          return DirectPage(
              child: ErrorBlock(message: snapshot.error, onRetry: reload));
        }
        ...
        final canManageEvent = AppData.text(event['created_by']).isNotEmpty &&
            AppData.text(event['created_by']) == (AppData.user?.id ?? '');

        return DirectPage(
            child:
                Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          PageHeader(
              title: AppData.text(event['title'], 'Evento'),
              subtitle: AppData.text(widget.group['name'], 'Grupo'),
              leading: true),
          const SizedBox(height: 12),
          PremiumEventDetailHero(
              event: event, date: date, yes: yes, minPeople: minPeople),
          if (eventIsTournamentEvent(event)) ...[
            ...
            AppCard(
              ...
              child: Row(children: [
                ...
                const Expanded(
                    child: Text(
                        'Este plan viene de Torneos. Puedes abrir la competición para cambiar resultado, fecha o jornada.',
                        style: TextStyle(
                            color: AppColors.ink,
                            fontWeight: FontWeight.w800,
                            height: 1.25))),
            ...
            SecondaryButton(
                ...
                onTap: saving ? () {} : () => openLinkedTournament(event)),
          ],
          ...
          if (eventIsRoutine(event)) ...[
            ...
            RoutineInfoBox(
                text:
                    '${eventRoutineBadge(event)} · al editar o cancelar podrás aplicar el cambio a una fecha, a futuras fechas o a toda la rutina.'),
          ],
          const SizedBox(height: 16),
          ...
          Row(children: [
            Expanded(
                child: AttendancePick(
                    ...
                    onTap: saving
                        ? () {}
                        : () => setStatus(event['id'].toString(), 'yes'))),
            ...
            Expanded(
                child: AttendancePick(
                    ...
                    onTap: saving
                        ? () {}
                        : () => setStatus(event['id'].toString(), 'maybe'))),
            const SizedBox(width: 8),
            Expanded(
                child: AttendancePick(
                    label: 'No voy',
                    ...
                    onTap: saving
                        ? () {}
                        : () => setStatus(event['id'].toString(), 'no'))),
          ]),
          if (saving)
            const Padding(
            ...
            PrimaryButton(
                ...
                onTap: () {
                  openAddressInGoogleMaps(
                      context, AppData.text(event['location']));
                }),
          ],
          if (canManageEvent) ...[
            const SizedBox(height: 18),
...
List<String> eventContributionSuggestions(Map<String, dynamic> event) {
  ...
  if (joined.contains('fiesta') ||
      joined.contains('cumple') ||
      joined.contains('karaoke')) {
    return const [
      'Bebida',
      'Hielo',
      'Vasos',
...
Future<String?> showEventContributionDialog(BuildContext context,
    {required String initial, required Map<String, dynamic> event}) async {
      ...
      builder: (context, setLocalState) => AlertDialog(
        ...
        content: SingleChildScrollView(
          child: Column(
              ...
              children: [
                const Text(
                    'Escribe algo sencillo. Por ejemplo: bebida, tortilla, pelotas o altavoz.',
                    style: TextStyle(
                        color: AppColors.muted,
                        fontWeight: FontWeight.w700,
                        height: 1.35)),
                ...
                if (hasInitial) ...[
                  ...
                  TextButton.icon(
                    onPressed: () => Navigator.pop(
                        dialogContext, _eventContributionDeleteToken),
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Container(
            ...
            decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.circular(16),
                border: Border.all(color: const Color(0x22D69027))),
          ...
          Expanded(
              child: Column(
                ...
                Text(
                  count == 0
                      ? 'Nadie ha dicho todavía qué lleva.'
                      : '$count ${count == 1 ? 'persona ya ha dicho qué lleva' : 'personas ya han dicho qué llevan'}.',
                  style: const TextStyle(
                      color: AppColors.muted,
                      fontWeight: FontWeight.w700,
                      height: 1.3),
        ...
        if (contributions.isEmpty)
          ...
          ...contributions.map((contribution) {
            final isMine =
                AppData.text(contribution['user_id']) == currentUserId;
            ...
            final text =
                AppData.text(contribution['items_text'], 'Algo para el evento');
            return Padding(
...
class _CalendarTabState extends State<CalendarTab> {
  ...
  String get groupId => widget.groupId.trim().isNotEmpty
      ? widget.groupId.trim()
  ...
  Map<String, List<Map<String, dynamic>>> _eventsByDay(
    ...
    for (final list in map.values) {
      list.sort((a, b) {
        final da = DateTime.tryParse(a['starts_at']?.toString() ?? '') ??
            DateTime.fromMillisecondsSinceEpoch(0);
        final db = DateTime.tryParse(b['starts_at']?.toString() ?? '') ??
            DateTime.fromMillisecondsSinceEpoch(0);
  ...
  List<Map<String, dynamic>> _eventsForDay(
      Map<String, List<Map<String, dynamic>>> index, DateTime day) {
    return List<Map<String, dynamic>>.from(
        index[calendarDayKey(day)] ?? const <Map<String, dynamic>>[]);
  }

  int _eventsInMonthFromIndex(
      Map<String, List<Map<String, dynamic>>> index, DateTime targetMonth) {
    var count = 0;
  ...
  Future<void> load({bool silent = false}) async {
    ...
    if (id.isEmpty) {
      ...
      setState(() {
        ...
        refreshing = false;
        errorMessage =
            'No se ha podido identificar este grupo. Vuelve a Mis grupos y entra otra vez.';
    ...
    try {
      final rows =
          await AppData.events(id).timeout(const Duration(seconds: 12));
  ...
  Future<void> createFor(DateTime day) async {
    final ok = await Navigator.of(context).push<bool>(
      MaterialPageRoute(
          builder: (_) =>
              CreateEventScreen(group: widget.group, initialDate: day)),
    );
  ...
  Future<void> openEvent(Map<String, dynamic> event) async {
    await Navigator.of(context).push(MaterialPageRoute(
        builder: (_) => EventDetailScreen(event: event, group: widget.group)));
  ...
  Widget build(BuildContext context) {
    ...
    visibleEvents.sort((a, b) {
      final da =
          DateTime.tryParse(a['starts_at']?.toString() ?? '') ?? DateTime.now();
      final db =
          DateTime.tryParse(b['starts_at']?.toString() ?? '') ?? DateTime.now();
      return da.compareTo(db);
    });
    ...
    final weekDays = List<DateTime>.generate(
        ...
        (i) => DateTime(weekStart.year, weekStart.month, weekStart.day)
            .add(Duration(days: i)));
    final weekEventsCount = weekDays.fold<int>(
        0, (sum, day) => sum + _eventsForDay(byDay, day).length);
        ...
        child: ListView(
          ...
          children: [
            ...
            if (selectedEvents.isNotEmpty) ...[
              SectionHeader(
                  title: 'Planes del día', action: '${selectedEvents.length}'),
              const SizedBox(height: 10),
              AgendaSameDayCompactCard(
                  events: selectedEvents,
                  group: widget.group,
                  onChanged: reload,
                  title: agendaSelectedDayTitle(selected)),
              const SizedBox(height: 18),
            ],
            SectionHeader(
